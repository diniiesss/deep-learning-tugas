...
from tensorflow.keras.layers import SimpleRNN, LSTM, GRU, Dense, Bidirectional, Input, Conv2D, MaxPooling2D, Flatten
...
def compress_and_resize_image(file_storage: FileStorage, max_size=(800, 800), quality=70):
    """Memuat, mengubah ukuran, dan mengompres gambar dari FileStorage."""
    ...
    # Reset pointer FileStorage agar bisa dibaca lagi jika diperlukan (opsional)
    file_storage.seek(0) 
...
def detect_objects_yolo(image_path):
    ...
    if YOLO_MODEL is None:
        try:
            # Memuat model YOLOv8 nano yang sudah dilatih (pertama kali akan diunduh)
            YOLO_MODEL = YOLO('yolov8n.pt') 
        except Exception as e:
            return None, f"Gagal memuat model YOLOv8n. Pastikan 'ultralytics' terinstal: {e}"

    # Menjalankan inferensi pada gambar (conf=0.50 untuk hasil yang lebih terpercaya)
    results = YOLO_MODEL(image_path, save=False, conf=0.50) 
    
    # Mengambil gambar hasil (numpy array dengan kotak dan label)
    result_image_array = results[0].plot() 
    
    # Konversi array gambar (BGR format default YOLO) ke RGB PIL Image
    ...
    # Mengambil list nama objek yang terdeteksi
...
def tugas3_predict():
    if request.method == 'POST':
        try:
            ...
            # SYNTAX KOREKSI: Pastikan kamus ditutup dengan }
            hasil = {
                'symbol': stock_symbol, 
                'days': pred_days, 
...
def tugas4_predict():
    ...
    if request.method == 'POST':
        ...
        if file:
            # 1. Kompresi dan Resize File (Mengurangi ukuran file sebelum disimpan)
            # Karena FileStorage sudah dibaca oleh compress_and_resize_image, kita harus membuatnya bisa dibaca ulang.
            file.seek(0)
            compressed_buffer = compress_and_resize_image(file)
            
            # 2. Simpan buffer yang sudah dikompresi ke disk
            ...
            try:
                # 3. Jalankan deteksi YOLO pada file yang sudah dikompresi
                detected_objects, plot_html = detect_objects_yolo(file_path)
                
                # 4. Konversi gambar yang diupload ke base64 untuk ditampilkan
                # Buka file yang sudah dikompresi untuk konversi base64
                with open(file_path, 'rb') as f:
                    uploaded_img_base64 = base64.b64encode(f.read()).decode('utf-8')

                # 5. Hapus file yang diupload (membersihkan server)
                os.remove(file_path)
...
# =================================================================
# RUN FLASK APP (MODE HOSTING)
# =================================================================

if __name__ == '__main__':
    # Pastikan UPLOAD_FOLDER dibuat
    if not os.path.exists(app.config['UPLOAD_FOLDER']):
        os.makedirs(app.config['UPLOAD_FOLDER'])
        
    # Mengambil PORT dari environment variable (misalnya dari Railway atau Heroku)
    port = int(os.environ.get("PORT", 5000))
    
    # Host '0.0.0.0' agar server dapat diakses dari luar container/lingkungan hosting
